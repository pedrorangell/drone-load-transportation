...
//     ROS_INFO ("\tMarker Pattern Filename: %s", pattern_filename_);
  ...
  {   
	...
	//the parent frame of the link: image_msg
  void ARSinglePublisher::getTransformationCallback (const sensor_msgs::ImageConstPtr & image_msg)
  {
    ...
    if (k != -1)
    {
      ...
      //3. Here we create a Transform object, and copy the information from marker pose into the 3D transform. 
      /********************************************/
      tf::Quaternion rotation (quat[0], quat[1], quat[2], quat[3]);
      tf::Vector3 origin(pos[0], pos[1], pos[2]);
      tf::Transform t(rotation, origin); //Transform Object "t"
      /********************************************/

	//Remember.... tf::TransformBroadcaster broadcaster_;

/*This is where the real work is done. Sending a transform with a TransformBroadcaster requires four arguments.
3.1 First, we pass in the transform itself: 						"t"
3.2 Now we need to give the transform being published a timestamp: 			"image_msg->header.stamp"
3.3 Then, we need to pass the name of the parent frame of the link we're creating: 	"image_msg->header.frame_id"
3.4 Finally, we need to pass the name of the child frame of the link we're creating: 	"markerFrame_.c_str()"
*/ 
//Turtle example: tf::StampedTransform markerToCam(t, ros::Time::now(), "world", turtle_name)
