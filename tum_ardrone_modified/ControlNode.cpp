...
{	
	//Drone Position and Control commands exchange
	...
	//Listens to specific commands (takeoff, land, reset)
	...
	// Channels
	/*drone pose*/
	dronepose_sub = nh_.subscribe(dronepose_channel, 10, &ControlNode::droneposeCb, this); //droneposeCb
	/*control*/
	...
	tum_ardrone_sub	   = nh_.subscribe(command_channel,50, &ControlNode::comCb, this); //comCb
...
//Callback function to get Drone Pose (receives statePtr)
...
void ControlNode::droneposeCb(const tum_ardrone::filter_stateConstPtr statePtr)
{
	...
	while(currentKI == NULL && commandQueue.size() > 0)
		popNextCommand(statePtr); //popNextCommand(filter_state)

	// if there is no current KI now, we obviously have no current goal -> send drone hover
	if(currentKI != NULL)
	{
		// let current KI control.
		this->updateControl(statePtr); //updateControl(filter_state)
	}
	else if(isControlling)
	{
		sendControlToDrone(hoverCommand); //sendControlToDrone(ControlCommand)
...
void ControlNode::Loop()
{
	...
	while (nh_.ok())
	{
		...
		//callAvailable() will take everything currently in the queue and invoke all of them. 
...
//Callback function for dynamic recunfigurable parameters
void ControlNode::dynConfCbPilot(tum_ardrone::AutopilotParamsConfig &config, uint32_t level)
{
...
/*}
void ControlNode::dynConfCbLoad(tum_ardrone::LoadControlParamsConfig &config, uint32_t level)
{*/
	//	Parameters for load control
	controller.Ki_rp_load = config.Ki_rp_load;
	controller.Kp_rp_load = config.Kp_rp_load;
	controller.max_rp_load = config.max_rp_load;
}

//Command Publisher callback function: tum_ardrone_pub.publish(string)
...
//updateControl(filter_state) uses update(statePtr)
