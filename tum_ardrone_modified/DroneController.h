...
//Control Command Structure (yaw, roll, pitch, gaz)
...
//Drone Position Structure (yaw, x, y, z)
...
private:
	...
	// Used for integral term (0 = target has been reached before, 1 = target is new)
	TooN::Vector<4> targetNew;

	// To keep track of integral terms (because i_term = error + last error)
	TooN::Vector<4> i_term;
	TooN::Vector<4> last_err;
	TooN::Vector<4> speedAverages;
	TooN::Vector<2> i_term_load; //********** load
	TooN::Vector<2> last_load_err; //********** load
	...
	void calcControl(TooN::Vector<4> new_err, TooN::Vector<4> d_error, double yaw, TooN::Vector<2> load_new_error);
...
public:
	...
	// PI control parameters for load stabilization ('K' iss upper-case)
	bool UseLoadControl;
	double Ki_rp_load;
	double Kp_rp_load;
	double max_rp_load;
