...
EstimationNode::EstimationNode()
{
	...
	/***Only fron camera*****/
    video_channel_ = nh_.resolveName("ardrone/front/image_raw");
	...
	last_pose[1] = 0;
	last_pose[2] = 0;
	lastPose_time = ros::Time(0);
	tf_obs = 0;
	selectedCam = 0;
	
	for(int kb=0;kb<bufferSize;kb++){
	circBuffer[kb][0]=0;
	circBuffer[kb][1]=0;}
	...
	vid_sub          = nh_.subscribe(video_channel_,10, &EstimationNode::vidCb, this);
	
	//Objeto que publica a postura do drone (mensagem do tipo filter_state)
...
void EstimationNode::Loop()
{
	  ...
	  while (nh_.ok())
	  {
		...
		// -------------- 2. get predicted drone pose ---------------
		// get filter_state msg S
		pthread_mutex_lock( &filter->filter_CS );
		tum_ardrone::filter_state s = filter->getPoseAt(ros::Time().now() + predTime); //postura(t + delta_controle)
		//pthread_mutex_unlock( &filter->filter_CS );

		// -------------- 3. get the load position ---------------

		/*Here, the real work is done, we query the listener for a specific transformation. The arguments are:
		-Target Frame (we want the transform from this frame..)
		-Parent Frame (..to this frame)
		...
		/*ros::ServiceClient client = nh_.serviceClient<ardrone_autonomy::CamSelect>("/ardrone/setcamchannel");
		ardrone_autonomy::CamSelect srv1;
		selectedCam = selectedCam + 1;
		if (selectedCam%29==0&& srv1.request.channel!=1){
			srv1.request.channel = 1;
			client.call(srv1);
			//ros::Duration(0.001).sleep(); 	
		}
		if (selectedCam%36==0){
			srv1.request.channel = 0;
			client.call(srv1);
			//ros::Duration(0.001).sleep();	
		}*/

		//Initialize circular buffer with the last load position
		circBuffer[tf_obs][0] = last_pose[0];
		circBuffer[tf_obs][1] = last_pose[1];
		marker_pose[2] = last_pose[2];
		
		/*The canTransform() methods return a bool whether the transform can be evaluated. 
		If it can't, the algorithm proceeds with the last position acquired*/
		if (tf_sub.canTransform("/ar_marker", "/world", ros::Time(0), NULL) == true) {
			try{
			tf_sub.lookupTransform("/ar_marker", "/world", ros::Time(0), marker_transform); //"/ardrone_base_bottomcam"
			//The lookupTransform() method is explained above;
			}
			...
			/*-----To not use the filter, comment the rest and leave only this:*
			marker_pose[0] = marker_transform.getOrigin().x();
			marker_pose[1] = marker_transform.getOrigin().y();----*/

			/*-----If using the filter, the transform goes to the current buffer location----*/
			//circBuffer[tf_obs][0] = marker_transform.getOrigin().x();
			//circBuffer[tf_obs][1] = marker_transform.getOrigin().y();
			/*-----Limitting the load position variation to +-1cm---comment this if prefer not to use step limitation---*/
			circBuffer[tf_obs][0] = std::min(last_pose[0]+0.02, std::max(last_pose[0]-0.02,marker_transform.getOrigin().x()));
			circBuffer[tf_obs][1] = std::min(last_pose[1]+0.02, std::max(last_pose[1]-0.02,marker_transform.getOrigin().y()));
			marker_pose[2] = marker_transform.getOrigin().z();
		}
		//if (selectedCam%10==0 && srv1.request.channel ==1) {
		//srv1.request.channel = 0;
		//client.call(srv1);}
		
		//Incrementing buffer pointer
		tf_obs = tf_obs +1;
		if (tf_obs==bufferSize) tf_obs = 0;

		//Calculating marker pose by average buffer value. 
		/*----If filter is not used the buffer will be filled with only the last position acquired----*/
		marker_pose[0] = 0;
		marker_pose[1] = 0;
		for (int jb=0;jb<bufferSize;jb++){
			marker_pose[0] = marker_pose[0] + circBuffer[jb][0];
			marker_pose[1] = marker_pose[1] + circBuffer[jb][1];
		}
		marker_pose[0] = marker_pose[0]/bufferSize;
		marker_pose[1] = marker_pose[1]/bufferSize; /**/

		//Limitting the time variation between consecutive transformations to minimum 33ms (avoid speed overshoots)
		//Marker_secs = (marker_transform.stamp_ - lastPose_time).toSec(); 		//Without limitation
		//Marker_secs = std::max(0.033,(ros::Time::now() - lastPose_time).toSec()); 	// With limitation
		Marker_secs = 0.033;								//Fixed	
		
		//Calculatin load speed			
		marker_speed[0] = (marker_pose[0] - last_pose[0])/(Marker_secs);
		marker_speed[1] = (marker_pose[1] - last_pose[1])/(Marker_secs);
		
		//Updating last values
		lastPose_time = ros::Time::now();//or marker_transform.stamp_;
		last_pose[0] = marker_pose[0];
		last_pose[1] = marker_pose[1];
		last_pose[2] = marker_pose[2];
		
		/**********************/
		pthread_mutex_unlock( &filter->filter_CS );

		//fill load metadata
		s.x_load = marker_pose[0];
		s.y_load = marker_pose[1];
		s.z_load = marker_pose[2];
		s.dx_load = marker_speed[0];
		s.dy_load = marker_speed[1];
...
/********************************************************************************************************************************/
		...
		// fill drone metadata
	  	...
	  	s.ptamState = ptamWrapper->PTAMStatus;	//estado PTAM
	  	s.droneState = lastNavdataReceived.state;	//estado sensores
