...
const std::string cameraImageTopic_ = "/ardrone/bottom/image_raw";
const std::string cameraInfoTopic_  = "/ardrone/bottom/camera_info";
...
namespace ar_pose
{
  ...
  private:
    ...
    ros::NodeHandle n_;
    ros::Subscriber sub_;
    //2.TransformBroadcaster object that we'll use later to send the transformations over the wire
    /********************************************/
    ...
    std::string markerFrame_; //name of the child frame of the link
    bool publishTf_;
